...
import { ConfirmNotifyPopup, AlreadyNotifiedPopup } from './UpcomingCardPopups';
...
export function UpcomingCard({ s }: { s: UpcomingLive }) {
  ...
  return (
    <>
        ...
        <div className="flex flex-1 flex-col gap-[8px] px-[14px] py-[12px]">
          <div className="flex items-center gap-[8px]">
            ...
            <div className="min-w-0">
              ...
              <p className="truncate font-outfit text-[11px] text-foreground/75">@{s.username}</p>
            </div>
          </div>
      ...
      {showAlready && (
        <AlreadyNotifiedPopup username={s.username} onClose={() => setShowAlready(false)} />
      )}
    </>
  );
}
